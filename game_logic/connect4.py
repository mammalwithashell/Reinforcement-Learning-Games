...
class Connect4Screen(Screen):
    ...
    def load_settings(self, diff=None, match=None, reset=False):
        ...
        if self.match_type == "League Match":
            ...
            if reset == False:
                ...
                league_qtable = resource_find(
                    "game_logic\connect4AI\qtables\league.txt"
                )
                ...
                board_agents.append(
                    Agent(
                        self.board_env,
                        resource_find(self.select_difficulty(diff)),
                        "max",
                    )
                )
    ...
    def play_game(self, num=None):
        ...
        if not self.board_env.is_full():
            ...
            # switch players
            self.board_env.turn = (
                "X" if self.board_env.turn == "O" else "O"
            )  # switch turn
    ...
    def select_difficulty(diff):
        diffdict = {
            "Easy": r"game_logic/connect4AI/qtables/easy.txt",
    ...
    def auto_select_difficulty(self):
        x = 0
        diffdict = {
            1: r"game_logic/connect4AI/qtables/easy.txt",
    ...
    def update_board(self):
        ...
        for i in range(len(board_str)):
            self.board[4 - int(i / 5)][i % 5] = (
                None if board_str[i] == "-" else board_str[i]
            )
            ...
            for i in range(5):
                ...
                if self.board[4 - j][i] == self.piece:
                    self.board_grid.add_widget(
                        Image(
                            source=resource_find("images/connect4/bestchipyellow.png")
                        )
                    )
                # adding red piece if current space belongs to AI
                elif self.board[4 - j][i] == self.opponent:
                    self.board_grid.add_widget(
                        Image(source=resource_find("images/connect4/bestchipred.png"))
                    )
